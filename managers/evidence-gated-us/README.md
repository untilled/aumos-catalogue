# Evidence-Gated Allocator — United States

`evidence-gated-us` is the **US sleeve**: it researches XNAS/XNYS and operates that sleeve,
including USD cash and policy-designated SGOV liquidity, inside the Global budget.
...
It handles `PORTFOLIO_REVIEW`, `ASSET_REVIEW`, `THESIS_REVIEW` and `EVENT_REVIEW`, and proposes a
single-asset BUY/SELL/RESIZE. A cross-market `REBALANCE` is `evidence-gated-global`'s and this
package never submits one. It submits exactly one AMP/1
...
This package is one of three, published together as the **Evidence-Gated Allocator** collection.
...
**One package is one manager.** Each is installed on its own, judged on its own and keeps its own
private memory — installing all three is a suggestion the catalogue makes, never one transaction, so
one of them being refused leaves the other two running. They collaborate through Evidence, Thesis,
Brief and WATCH; a specialist cannot borrow the other market's budget and raises a shared Brief/WATCH
agenda for Global instead. An urgent thesis-invalidation exit does not wait for the next Global run.

⚠️ **Nothing requires the other two to be installed.** This package is complete on its own: with no
Global sleeve budget in Brief it operates on what the mandate and the book say, and it never assumes
a sibling has run.
