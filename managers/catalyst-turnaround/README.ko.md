...
flowchart TB
    ...
    subgraph IN["읽는 것"]
        ...
        LED["자기 후보 장부<br/>조사 중인 종목, 미확인 질문,<br/>마지막으로 읽은 접수번호"]:::reads
    ...
    subgraph SWEEP["미보유면 먼저 찾고, 무엇을 봤는지 적는다"]
        direction TB
        S1["재개 — 실패한 접수 범위를 다시 시도하고<br/>남은 조사를 끝낸다"]:::judges
        S2["유니버스와 공시 창을 선언한다"]:::judges
        S3["마지막 성공 접수번호 이후의<br/>새 공시와 정책 원문을 증분 수집한다"]:::judges
        S4["최대 3종 기초 조사, 최소 1종 완결,<br/>그리고 이번 실행이 넷 중 무엇인지 기록:<br/>candidates_produced · no_candidate_qualified ·<br/>discovery_not_run · discovery_incomplete"]:::judges
    end
...
| **데이터 소스** | 미수금·운전자본·부채·현금흐름 항목을 위한 `open-dart` 공시와 정책 원문. 호스트 자신의 OpenDART 소스 캐시를 통해 읽으며 두 경로를 모두 씁니다. 종목코드를 공시가 키로 쓰는 식별자로 바꿔 주는 기업코드 목록, 그리고 그것으로 읽는 공시 목록입니다. 연결된 Toss 시세 로그인은 수정 원화 가격 이력을 주고, **동시에** 이 매니저가 탐색 전에 선언하는 국내 상장 명부를 제공합니다. 호스트에서 시장 전체를 열거할 수 있는 것이 달리 없기 때문입니다. 부처 고시나 요금 결정 같은 웹 원문은 인용되기 전에 URL·발행자·발행일과 함께 증거로 등록됩니다 |
| **키 비용** | OpenDART 키는 금융감독원에 등록하면 무료입니다. 가격 연결은 이 펀드가 이미 가진 로그인이며, 시세만 쓴다면 증권 계좌가 붙어 있을 필요는 없습니다 |
| **자기 기억** | **두 개의 기록이고, 결코 합쳐지지 않습니다.** 하나는 촉매 원장입니다. 열린 창, 지연 횟수, 기록된 반대 증거이며, 이것이 없으면 세 번 지연된 포지션이 첫 지연처럼 보이므로 이 매니저는 추측하지 않고 읽지 못했다고 말합니다. 다른 하나는 그 옆의 후보 장부입니다. 조사 중인 종목, 각 종목에 남은 미확인 질문, 마지막으로 읽은 접수번호, 아직 재시도해야 할 실패 범위입니다. 어느 쪽도 가격·공시 원문·보유량·현금을 복제하지 않습니다. 그것들은 매 실행마다 계좌와 소스에서 다시 읽습니다 |
...
- **기다림.** 두 번 지연된 촉매는 1년치 자본을 쓰며, 어떤 명세서도 그 비용을 청구하지 않습니다.
  ...
  합니다.
- **애초에 찾아내는 일.** 입구는 공시나 고시에 적힌 사건이고, 그 탐색은 전수가 아니라 증분이며 예산과 재개
  지점이 있습니다. ⛔ **읽지 못한 공시 창은 「후보 없음」이 아니라 `discovery_not_run`으로 보고합니다.**
  두 문장은 똑같이 비어 있는 답을 내지만 정반대를 뜻하고, 시장에 대해 말할 수 있는 쪽은 유니버스를
  선언했고 필요한 레인이 모두 답했으며 실패한 범위가 없을 때뿐입니다. 아무 데도 보지 못했다고 정직하게
  보고하는 실행을 예상하십시오. 공시 예산을 보유 종목 검토에 전부 쓴 실행이 그중 하나입니다.
...
지목하고, 데이터 부재로 기록합니다. **그리고 총계를 만들 수 있는데 이미 그 한도를 넘겨 있으면
그때도 아무것도 개시하지 않고 분할 단계도 추가하지 않습니다** — 어느 업종이 얼마를 들고 있고
선언된 한도가 얼마인지를 말하며, 그 답은 회사가 아니라 계좌에 대한 것입니다. 한동안 둘 중 앞의
것만 배선되어 있었습니다: 확인할 수 없는 한도는 개시를 보류했고, 확인해서 넘긴 것으로 드러난
한도는 보류하지 않았습니다. 청산, 무효화에 따른 축소, 위험한도로의 재조정, 지연을 견디는 보유는
어느 쪽에도 영향받지 않습니다. **업종 한도가 보류하는 것은 증액이고**, 이 데스크가 자기 포지션을
얼마나 파는지는 결코 정하지 않습니다 — 남의 이름들이 채운 천장은 어느 이름이 내려와야 하는지에
