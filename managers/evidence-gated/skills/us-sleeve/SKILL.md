...
`us:<symbol>` and `outputPath` `scans/<asOf date>/<recipeId>`; `task_get` until it settles, then
...
`XNAS:`/`XNYS:` ids, every one `sourced: false` / `documents: 0` / `barsRead: 0`, beside 83 of 83
