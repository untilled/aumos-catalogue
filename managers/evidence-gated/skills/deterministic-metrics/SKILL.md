...
| `concentration` | `caps.portfolioHeat` — the Mandate's `maxDrawdown` — and `stopLossPct` + `core` on each row | Heat is loss-if-every-stop-fires, which weights do not measure. A non-core row with no stop is unevaluated, not zero. A Mandate that declares no drawdown limit leaves it `unevaluated`, which is not a pass. |
