...
| `mandateExecution` | `mandateObjective` — the invocation's `mandate.objective`, verbatim — plus `positions`, `cashWeight` and `reportedDiagnostics`, the codes this run's other operations already returned | ⛔ The objective is **carried and never parsed**: it is free prose, every investor writes a different one, and a run that read the words and inferred what the book should hold would be deciding an allocation from a label. Three checkable things are judged instead — whether an objective is on the record, whether any single-name weight is held, and which of `inputPathCodeVocabulary` this run's own diagnostics contain. Without `reportedDiagnostics` the cause is `unreported` and `unevaluated`, which is not a pass; matched input-path codes make it `input-path-incomplete`; `no-candidate-cleared-the-gates` / `info` is **earned, never fallen into** (#171) — it needs at least one `gateRanCodeVocabulary` member, a gate that actually ran and refused, because holding cash when nothing clears the gates is the methodology working and a run reporting codes this operation cannot read has not shown that. ⚠️ Codes reported and codes recognised are two numbers on the response (`reportedDiagnosticCount`, `recognisedCodes`): «many reported, none recognised» is `unreported`. ⛔ All three vocabularies are projections of `lib/diagnostic-codes.mjs`, where the code, the operation that emits it and the module that must contain it live on one row — pass the codes your siblings actually returned, verbatim, never a paraphrase of them. ⚠️ `thesisGapSources` supplies the sharpest input-path code there is — `valuation_gap_is_unfetched_not_unfillable` means a source that fills the gap **exists for this instrument and was never called**. ⛔ Its two siblings are read differently on purpose: `valuation_gap_has_no_source_for_this_instrument` is a fact about the instrument and no amount of wiring closes it, and `instrument_class_unknown` proves neither reading — it lands on `unresolvedCodeVocabulary`, which forbids the `info` answer without claiming the wiring is at fault. Unknown is not incomplete. ⚠️ It is a report: it never blocks, never asks for a purchase and never proposes disposing of parked liquidity. |
