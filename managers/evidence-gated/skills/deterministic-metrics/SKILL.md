...
| `executionRecord` | `prepared`, `job` and `result` — what `research_prepare`, `research_job_get` and `research_result_get` returned, **verbatim** — plus `eligibleSymbols`, the names your own fold found eligible | ⚠️ **The counts are the host's and the eligibility is yours.** `sourced`, `evaluated`, `unprepared` and `failed` come off the research summary and are ⛔ **never summed into a coverage number**; `eligibleCount` is derived from `eligibleSymbols` and ⛔ never typed, because a number you can type is a number you can invent. ⛔ **An absent list is `null` and an empty one is `0`** — «nobody folded the rows» and «the rows were folded and nothing cleared» are different facts and only the second is a measurement. ⚠️ A cache hit answers on `research_prepare` itself (`cached: true` with a `summary` and no job to poll), so handing only `result` would tell that run it had prepared nothing. ⛔ `sourced` is absent while a job is `unsettled`, deliberately: the settled summary derives it as items minus gaps, and deriving it mid-flight counts a name nobody has reached yet as one this fund can read — a count that goes down is a count a reader stops trusting. |
