...
- **선언한 크기로 개별종목을 사는 것 — 처음 몇 년 동안은.** ⚠️ **설치 전에 읽어라. 가장 놀라기
  쉬운 지점이다.** 펀드 설정에서 정한 개별종목 상한은 천장이지 실제로 작동하는 숫자가 아니다.
  렌즈가 *승격*되기 전까지 이 매니저는 모든 개별종목을 실험 상한 — **자산의 1% 안팎**, 어떤 경우에도
  3%를 넘지 않는다 — 으로만 산다. `maxPositionWeight`에 무엇이 적혀 있든 그렇다. 20%를 선언하고
  렌즈가 승격 전이면 실효 상한은 1%다. **선언값의 20분의 1이다.** 이제는 그것이 적용되는 모든 실행이
  그 사실을 소리 내어 말한다(`position_cap_reduced_by_maturity` — 선언값·실효값·무엇이 풀어주는지를
  함께 낸다). 이전에는 아무 데도 나타나지 않았다.
  **그리고 승격은 주 단위가 아니라 연 단위다.** 렌즈는 닫힌 결과 30건, 독립 날짜 클러스터 10개,
  그리고 **서로 다른 시장 레짐 3개**로 승격된다. 앞의 둘은 후보를 더 많이 찾으면 빨라지지만 셋째는
  아니다 — 레짐은 달력이 지나야 바뀌므로, 레짐 3개는 활동량으로 줄일 수 없는 수년짜리 대기다.
  그동안 개별종목은 1% 실험이고 장부의 무게는 ETF와 현금에 있다. **그 기간 동안 이것은 사실상
  ETF 배분기이고, 옆에서 작은 대조 실험을 하나 돌리는 것이다.** 이것은 결함이 아니라 설계대로
  작동하는 방법론이다 — 증거로 크기를 정하는 방법론은 증거가 쌓이기 전에는 작을 수밖에 없다 —
  그러나 당신이 설치하려는 것의 수년짜리 성질이고, 미리 들을 자격이 있다.
  이번 분기에 확신 하나에 장부의 20%를 실어 줄 매니저를 원한다면 이 매니저가 아니다.
- **공매도와 레버리지.** 롱온리다.
