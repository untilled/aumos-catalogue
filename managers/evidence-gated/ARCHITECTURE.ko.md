...
- **미래나 날짜를 담은 durable 키는 되읽을 수 없다.** `memory_read`는 ISO-8601 모양이면서 `asOf`
  ...
  담았으므로, 둘 다 정상 동작에서 거부됐다. 그리고 거부는 키 단위가 아니라 읽기 단위다: 오염된 키
  하나가 키 없는 네임스페이스 읽기 전체를 죽인다. 패키지 쪽 답은 타임스탬프 모양의 문자열을 쓰지
  않는 것이다 — 실제로 순간인 값에는 epoch ms, 애초에 순간이 아니었던 필드에는 `session-` 접두사
  라벨. 나머지 절반은 호스트의 것이다: 예정 키에는 면제가 필요하거나, 서빙할 수 없는 키만 떨구고
  나머지를 돌려주는 엔트리 단위 읽기가 필요하다.
