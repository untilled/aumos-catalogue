...
- `orchestrate`: 이 웨이크가 어느 플로우의 것인지, 단일 슬리브 실행이 무엇을 낼 수 있는지, 플로우를 어떻게 디스패치하는지;
...
- **장중 웨이크가 실제로 오는지는 런타임의 몫이지 이 패키지의 몫이 아니다.** `evaluateWatch`는
  어떤 조건이 실시간 가격으로 정직하게 채점될 수 있고 어떤 조건이 완성된 봉을 요구하는지 알고,
  후자를 전자만 가진 채로 `not-met`이라 부르기를 거부한다. 하지 못하는 것은 웨이크를 일으키는
  일이다 — Aumos가 걸린 price WATCH를 얼마나 자주 평가하는지는 이 저장소가 볼 수 없는 런타임
  속성이다. 장중에 평가한다면 원본 하네스의 야간 경로는 덮인다. 종가에만 평가한다면 패키지는
  여전히 옳고 — 관측을 받지 못할 뿐이다 — 그 공백은 패키지가 아니라 런타임에 대한 요청이 된다.
