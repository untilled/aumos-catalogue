...
| `screen_universe.json` · `universe_extensions.json` | AR | `coverageState({ scannerUniverses, extensions })` — the run declares the universe and passes it in. ⚠️ **This row said "Aumos owns the universe" and nothing did.** Aumos owns the holdings, the relay and the coverage denominator's other half; nobody procured a screen, so `coverage` was called with two empty lists and answered `complete: true` over the holdings alone (#129). The two files stay unported deliberately — a committed list is the drift these two were the record of — and what replaces them is a call per run: the listing route in `skills/data-source-contract/SKILL.md`, the procedure in `skills/candidate-research/SKILL.md`, and `universe_undeclared` in `lib/coverage.mjs` for the run that skips both |
