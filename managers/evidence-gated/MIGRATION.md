...
| `screen_universe.json` · `universe_extensions.json` · `docs/opportunity-universe.*.json` | PX / AR | #146 restores the pinned KR 74 / US 83 curated rosters under `data/`; `researchUniverse` combines them with dated, evidenced extensions and requires current eligibility checks. `coverage` still measures the declared scope, never the whole market. `researchState` persists a bounded roster and Evidence references under `coverage/research-index`. Raw fundamentals stay host-owned and are now **read** rather than re-procured: `fundamentalsPlan` → `source_cache_read` / `source_cache_refresh` (aumos#671, #683), with `mapCorporationCodes` supplying the `corp_code`/CIK the roster does not carry and `radarFeedDiagnosis` naming the stage when the branch still comes back unfed; see `HOST-FOLLOWUPS.md`. |
