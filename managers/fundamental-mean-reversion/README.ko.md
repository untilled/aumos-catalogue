...
| **연결** | 조정 일봉과 시장 캘린더를 위한, 이 펀드에 연결된 토스 로그인. 시세 전용 로그인으로 충분하며 증권계좌가 붙어 있을 필요는 없다 |
| **데이터 소스** | 공시를 위한 `open-dart`. 일회성 비용과 진짜 악화의 구분은 오직 거기서만 이뤄진다 |
...
| **기억** | 자기 분할 진입 원장과 미완성 조사 메모를 자기 폴더에 둔다. 보유·현금·체결은 Aumos의 것이며 그곳으로 복사되지 않는다 |
...
**최초 공개 시점의 알려진 한계.** 한 북에 여러 매니저가 붙었을 때의 중복 노출 귀속, 제안의 저장,
리뷰의 재기동, 결정과 실제 체결의 연결은 모두 호스트의 몫이고, 이 저장소의 검사는 그중 무엇도
실행하지 않는다. 실행 중인 Aumos에서 검증되기 전까지는, 같은 종목을 들 수 있는 다른 매니저와 함께
쓰는 것은 분리된 펀드에서 하는 편이 낫다.
